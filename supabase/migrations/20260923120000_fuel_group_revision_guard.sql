begin;
-- Optimistic concurrency for multi-step buttons; existing rows and grants preserved.
create or replace function public.fuel_group_step(p_company uuid,p_user uuid,p_group text,p_sender text,p_message text,p_action text,p_patch jsonb,p_revision integer default null,p_draft uuid default null,p_command jsonb default null,p_dry_run boolean default true) returns jsonb
...
begin
  ...
  if p_action <> 'confirm' and p_revision is not null and (p_revision <> s.revision or (p_draft is not null and p_draft <> s.draft_id)) then
    raise exception 'Rascunho alterado. Solicite RESUMO.' using errcode='40001';
  end if;
