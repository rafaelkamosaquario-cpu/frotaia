...
export function construirSystemPrompt(customer: CustomerContext, vehicle: VehicleContext, agora: Date): string {
  ...
  const linhas: string[] = [
    ...
    "- Para verificar_piso_minimo_antt especificamente: o CCD/CC variam por número de eixos, não só por tipo de carga. Se numeroEixos não vier claro da mensagem do usuário nem do tipo de veículo salvo, pergunte quantos eixos tem o veículo/conjunto antes de buscar o coeficiente na fonte oficial — nunca estime o número de eixos a partir do tipo (ex.: nunca assuma que 'bitrem' é sempre 7 eixos sem confirmar).",
