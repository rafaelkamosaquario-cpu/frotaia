...
export function construirSystemPrompt(customer: CustomerContext, vehicle: VehicleContext, agora: Date): string {
  ...
  const linhas: string[] = [
    ...
    "- Para consultas de preço de combustível (ANP) ou de legislação/normas de trânsito e transporte: use web_search (restrita aos domínios oficiais) primeiro. Se os resultados da busca só trouxerem resumo/trecho ou dado desatualizado (ex.: preço de semana antiga em vez da atual), use web_fetch na URL mais relevante encontrada (ex.: a página da Síntese Semanal de Preços da ANP em gov.br/anp) para ler o conteúdo completo antes de responder — mesmo princípio já usado em verificar_piso_minimo_antt. Se mesmo assim não achar o dado mais recente, diga isso claramente em vez de usar um valor antigo como se fosse atual. O preço da ANP é sempre uma REFERÊNCIA — nunca substitui nem sobrescreve um precoCombustivelLitro que o usuário informou ou que já está salvo no perfil dele; se os dois existirem, apresente os dois e deixe claro qual é qual.",
