...
export function construirSystemPrompt(customer: CustomerContext, vehicle: VehicleContext, agora: Date): string {
  ...
  const linhas: string[] = [
    ...
    "- Para verificar_piso_minimo_antt: NUNCA invente ou lembre de memória o coeficienteDeslocamentoReaisPorKm (CCD) ou o custoCargaDescargaReais (CC) — são valores oficiais que mudam por resolução da ANTT. Antes de chamar essa ferramenta, use web_search (restrita aos domínios oficiais) para encontrar o CCD/CC vigente pro tipo de carga e número de eixos informados pelo usuário (ex.: consultar calculadorafrete.antt.gov.br ou a resolução mais recente em antt.gov.br), preencha fonteCoeficiente com a resolução encontrada, e só então calcule. Se a busca não encontrar um valor claro, diga isso ao usuário em vez de estimar. Deixe sempre claro que este é o piso LEGAL (Lei 13.703/2018) — nunca confunda com o piso econômico de calcular_valor_minimo_frete, que é outro cálculo.",
    "- Para consultas de preço de combustível (ANP) ou de legislação/normas de trânsito e transporte: use web_search (restrita aos domínios oficiais) para trazer a informação mais atual. O preço da ANP é sempre uma REFERÊNCIA — nunca substitui nem sobrescreve um precoCombustivelLitro que o usuário informou ou que já está salvo no perfil dele; se os dois existirem, apresente os dois e deixe claro qual é qual.",
