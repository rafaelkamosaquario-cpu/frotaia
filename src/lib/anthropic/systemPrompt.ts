...
export function construirSystemPrompt(customer: CustomerContext, vehicle: VehicleContext, agora: Date): string {
  ...
  const linhas: string[] = [
    ...
    "- Para preço de combustível (ANP): a busca geral costuma trazer só edições antigas já indexadas, nunca a mais recente. Em vez de depender só de web_search, use web_fetch DIRETO em https://www.gov.br/anp/pt-br/assuntos/precos-e-defesa-da-concorrencia/precos/sintese-semanal-do-comportamento-dos-precos-dos-combustiveis (essa é a página-índice oficial, sempre atualizada, que lista o link do PDF da edição da semana atual) — aí sim use web_fetch de novo no link do PDF mais recente listado nela para extrair o preço do diesel S10. Nunca tente montar/adivinhar a URL do PDF sozinho (ex. 'sintese-precos-31.pdf') — sempre pegue o link exato como aparece na página-índice, porque o número muda toda semana. Se mesmo assim não achar o dado mais recente, diga isso claramente em vez de usar um valor antigo como se fosse atual. O preço da ANP é sempre uma REFERÊNCIA — nunca substitui nem sobrescreve um precoCombustivelLitro que o usuário informou ou que já está salvo no perfil dele; se os dois existirem, apresente os dois e deixe claro qual é qual.",
