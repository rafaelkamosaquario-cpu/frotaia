...
1. **P3 — `consultar-rota.ts:93-96`**: repassa `err.message` de `GoogleMapsApiError` (pode conter `status` bruto do Google, ex. `OVER_QUERY_LIMIT`) diretamente ao usuário, diferente do padrão de mensagem genérica fixa usado nas outras 15. Não é PII/SQL, é status técnico de API externa — risco baixo, mas quebra a consistência do padrão de segurança do resto do código.
2. **P2 — `gerenciar-documento-frota.ts`**: não verifica se `motoristaId` pertence à `companyId` antes de CRIAR/ATUALIZAR um documento vinculado a ele (só verifica quando é `veiculoId`). Não vaza leitura entre empresas (o documento é sempre gravado com o `company_id` correto), mas permite, em teoria, vincular um documento da empresa A a um `driver_id` de outra empresa se alguém souber/adivinhar o UUID. Recomendação: replicar a checagem de posse já usada nas outras 6 ferramentas que lidam com veículo.
3. **P3 — padrão "leitura ampla por `id` + checagem manual de `company_id` em JS"** usado em `getVehicle`, `getRoute`, `getSavedJourney` (sem `.eq("company_id", ...)` na própria query de serviço). Seguro hoje porque toda ferramenta que os chama confere posse logo em seguida, mas é dívida técnica frágil — um novo caminho de código que reutilize essas funções sem repetir a checagem reintroduziria vazamento cross-tenant silenciosamente. Recomendação: mover o filtro `company_id` para dentro das funções de serviço.
4. **P3 — sem trava de desambiguação em código** (ver acima) — depende 100% do system prompt ser seguido pelo modelo.

Nenhum P0/P1 encontrado nas 28 ferramentas. Nenhuma correção foi aplicada — aguardando decisão sobre os achados 1-4 acima.
